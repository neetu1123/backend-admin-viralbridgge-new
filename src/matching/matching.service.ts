...
type CampaignWithBrand = {
  ...
  languages: string[];
  brand?: { industry: string | null; company_name: string } | null;
};
...
export class MatchingService {
  ...
  async runMatchingForCampaign(campaignId: string) {
    ...
    if (!campaign || campaign.status !== 'ACTIVE') return { created: 0, skipped: true };
  ...
  async runMatchingForAllActiveCampaigns() {
    const enabled = await this.isAiMatchingEnabled();
    if (!enabled) return { totalCreated: 0, campaigns: 0, enabled: false };
  ...
  async updateMatchStatus(id: string, status: 'active' | 'removed' | 'forced', adminId?: string) {
    const enabled = await this.isAiMatchingEnabled();
    if (!enabled) {
      throw new ForbiddenException('AI matching is disabled');
    }
  ...
  async getCreatorCampaignMatchScores(creatorId: string, campaignIds: string[]) {
    const enabled = await this.isAiMatchingEnabled();
  ...
  async getCampaignRecommendations(campaignId: string, brandUserId: string) {
    const enabled = await this.isAiMatchingEnabled();
    if (!enabled) {
    ...
    const recommendations = stored
      ...
      .map((m) => ({
        ...
        verified: Boolean((m.creator.user as { is_verified?: boolean } | undefined)?.is_verified),
      }));
