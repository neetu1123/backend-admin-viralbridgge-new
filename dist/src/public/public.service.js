...
function buildUsername(name, email, id) {
...
function getPrimaryPlatform(socialLinks) {
...
function getCampaignStatus(applicants, deadline) {
    ...
    if (daysLeft <= 7)
        return { status: 'Closing Soon', color: '#F9A826', bg: '#FFF8EC' };
    if (applicants >= 100)
        return { status: 'Hot', color: '#F357A8', bg: '#FFF0F6' };
...
let PublicService = class PublicService {
    ...
    async listCreators(query) {
        ...
        const orderBy = query.sort === 'followers' || query.sort === 'followers_desc'
            ? { followers: 'desc' }
    ...
    async getCreatorByUsername(username) {
        ...
        if (isUuid(normalized)) {
            ...
            if (byId)
                return this.formatCreatorDetail(byId);
        }
    ...
    async listCampaigns(query) {
        ...
        const orderBy = query.sort === 'budget_desc' || query.sort === 'budget_high'
            ? { budget: 'desc' }
    ...
    async getPlatformStats() {
        const [verifiedCreators, activeBrands, liveCampaigns, campaignsCompleted, premiumMembers,] = await Promise.all([
    ...
    formatCreatorListItem(row) {
        ...
        const platform = getPrimaryPlatform(row.social_links);
        ...
        return {
            ...
            bio: row.bio || '',
            tags: niche.split(/[,/&]+/).map((t) => t.trim()).filter(Boolean).slice(0, 3),
    ...
    formatCreatorDetail(row) {
        ...
        const brandsWorkedWith = [
            ...new Set(row.applications?.map((app) => app.campaign?.brand?.company_name).filter(Boolean) ?? []),
        ];
        return {
            ...base,
            coverImage: '',
            ...
            socialLinks,
            brandsWorkedWith,
            recentCampaigns: row.applications?.map((app) => ({
                id: app.campaign?.id,
    ...
    formatCampaignDetail(row) {
        ...
        return {
            ...base,
            creatorRequirements: row.metadata?.requirements ?? '',
