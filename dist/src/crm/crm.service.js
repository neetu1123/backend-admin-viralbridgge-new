...
let CrmService = class CrmService {
    ...
    resolveFollowUpStatus(date, time, completed) {
        if (completed)
            return 'completed';
        const now = new Date();
    ...
    buildWhere(query) {
        ...
        if (query.dateFrom)
            where.created_at = { ...where.created_at, gte: new Date(`${query.dateFrom}T00:00:00.000Z`) };
        if (query.dateTo)
            where.created_at = { ...where.created_at, lte: new Date(`${query.dateTo}T23:59:59.999Z`) };
        return where;
    }
    buildOrderBy(sort) {
        switch (sort) {
    ...
    async getSummary() {
        ...
        const [totalLeads, newLeads, qualifiedLeads, convertedLeads, lostLeads, todaysFollowUps] = await Promise.all([
    ...
    async getLeads(query) {
        ...
        if (query.sort === 'priority') {
            leads = leads.sort((a, b) => (PRIORITY_ORDER[b.priority] ?? 0) - (PRIORITY_ORDER[a.priority] ?? 0));
        }
    ...
    async createLead(dto, userId, userName) {
                ...
                follow_ups: dto.nextFollowUpDate
                    ? {
                        create: {
                            ...
                            status: this.resolveFollowUpStatus(dto.nextFollowUpDate, dto.nextFollowUpTime || '10:00', false),
                        },
                    }
                    : undefined,
            },
    ...
    async updateLead(id, dto, userName) {
    ...
    async addNote(leadId, content, createdBy) {
    ...
    async updateNote(leadId, noteId, content) {
    ...
    async completeFollowUp(leadId, followUpId) {
