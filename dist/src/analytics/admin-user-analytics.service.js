...
exports.AdminUserAnalyticsService = void 0;
...
let AdminUserAnalyticsService = class AdminUserAnalyticsService {
    ...
    async listUsers(query) {
        ...
        const data = users.map((u) => {
            ...
            const campaignCount = roleName === 'BRAND'
    ...
    async getUserWallet(userId, query) {
        ...
        const txWhere = { wallet_id: wallet.id };
        if (query.from || query.to) {
            txWhere['created_at'] = {
                ...(query.from ? { gte: new Date(query.from) } : {}),
                ...(query.to ? { lte: new Date(query.to) } : {}),
    ...
    async getUserActivity(userId, query) {
        ...
        const addToMonth = (date, field) => {
            const key = (0, analytics_date_util_1.monthKey)(date);
            const bucket = monthlyMap.get(key) ?? { campaigns: 0, applications: 0, messages: 0 };
            bucket[field] += 1;
            monthlyMap.set(key, bucket);
        };
        for (const c of campaigns)
            addToMonth(c.created_at, 'campaigns');
        for (const a of applications)
            addToMonth(a.created_at, 'applications');
        for (const m of messages)
            addToMonth(m.created_at, 'messages');
    ...
    async getUserCampaigns(userId, query) {
        ...
        if (roleName === 'BRAND') {
            ...
            if (!brand)
                return { campaigns: [], summary: { created: 0, active: 0, completed: 0, successRate: 0 } };
        ...
        if (!creator)
            return { applications: [], summary: { submitted: 0, approved: 0, rejected: 0 } };
    ...
    async getCreatorMetrics(userId) {
        const creator = await this.prisma.creatorProfile.findUnique({ where: { user_id: userId } });
        if (!creator)
            return null;
    ...
    async getBrandMetrics(userId) {
        const brand = await this.prisma.brandProfile.findUnique({ where: { user_id: userId } });
        if (!brand)
            return null;
        const [campaignSpend, invitations, applicationsReceived, topCampaigns, completed, total] = await Promise.all([
    ...
    async getEarningsTrend(userId) {
        ...
        for (const tx of txs) {
            const key = (0, analytics_date_util_1.monthKey)(tx.created_at);
            monthly.set(key, (monthly.get(key) ?? 0) + tx.amount);
        }
        return [...monthly.entries()]
            .sort(([a], [b]) => a.localeCompare(b))
            .map(([key, earnings]) => ({ month: (0, analytics_date_util_1.formatMonthLabel)(key), key, earnings }));
    }
    async buildCampaignGrowth(profileId, type) {
        const campaigns = await this.prisma.campaign.findMany({
        ...
        return [...monthly.entries()]
            .sort(([a], [b]) => a.localeCompare(b))
            .map(([key, count]) => ({ month: (0, analytics_date_util_1.formatMonthLabel)(key), key, count }));
    }
    calcProfileCompletion(user) {
        const roleName = user.role?.name ?? '';
...
exports.AdminUserAnalyticsService = AdminUserAnalyticsService;
exports.AdminUserAnalyticsService = AdminUserAnalyticsService = __decorate([
    (0, common_1.Injectable)(),
    __metadata("design:paramtypes", [prisma_service_1.PrismaService])
], AdminUserAnalyticsService);
//# sourceMappingURL=admin-user-analytics.service.js.map